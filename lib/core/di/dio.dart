...
class RemovalInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    final accessToken =
        await jwtStore.load(properties: JwtStoreProperties.accessToken);
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    final refreshToken =
        await jwtStore.load(properties: JwtStoreProperties.refreshToken);
    final accessToken =
        await jwtStore.load(properties: JwtStoreProperties.accessToken);
    ...
    if (err.response?.statusCode == 401) {
      try {
        ref
            .read(reIssueViewModelProvider.notifier)
            .reIssue(accessToken: accessToken);
